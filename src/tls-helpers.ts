...
export function pumpSocket(socket: Socket, tls: { handleReceivedBytes(b: Uint8Array): void }, leftover: Uint8Array): void {
	...
	(async () => {
		try { while (true) { const { value, done } = await reader.read(); if (done) break; tls.handleReceivedBytes(value); } }
		catch { /* socket closed */ }
...
export function makeTlsReadable(s: TlsState, hd: Promise<void>, close: () => void): ReadableStream<Uint8Array> {
	return new ReadableStream({
		async pull(controller) {
			await hd;
			if (s.tlsError) { controller.error(s.tlsError); return; }
			if (s.chunks.length > 0) { controller.enqueue(s.chunks.shift()!); return; }
			if (s.tlsEnded) { controller.close(); return; }
			await new Promise<void>((r) => { s.resolveAppData = r; });
			if (s.tlsError) { controller.error(s.tlsError); }
			else if (s.chunks.length > 0) { controller.enqueue(s.chunks.shift()!); }
